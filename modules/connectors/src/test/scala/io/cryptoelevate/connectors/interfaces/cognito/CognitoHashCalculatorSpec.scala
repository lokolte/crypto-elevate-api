...
object CognitoHashCalculatorSpec extends DefaultRunnableSpec {
    ...
    override def loadConfig(): Task[Config.CEConfig] = Task.succeed {
      CEConfig(
        aws = AwsConfig(
          ...
          dynamo = DynamoConfig("dummy table"),
          ses = SesConfig("url", "http", "ii"),
          redis = RedisConfig(url = "test", port = 3469),
          region = "xy-qwerty-1",
          poolId = "1234567890"
        ),
